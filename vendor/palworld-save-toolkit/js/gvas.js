...
export class FReader {
  ...
  structValue(structType, path = "") {
    ...
    // FColor is 8-bit BGRA, in that byte order (LocalData.sav paint palettes)
    if (structType === "Color") return { b: this.byte(), g: this.byte(), r: this.byte(), a: this.byte() };
...
export class FWriter {
  ...
  structValue(structType, value) {
    ...
    else if (structType === "Color") { this.byte(value.b); this.byte(value.g); this.byte(value.r); this.byte(value.a); }
...
export const PALWORLD_TYPE_HINTS = {
  // LocalData.sav (client-side world state)
